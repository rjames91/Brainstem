...
noise = generate_signal(signal_type='noise', dBSPL=dBSPL, duration=0.1,
                        modulation_freq=0., modulation_depth=0.5, fs=Fs, ramp_duration=0.005, plt=None, silence=True,
...
if conn_pre_gen:
    try:
        connection_dicts_file = np.load(input_directory + '/cn_{}x{}an_fibres_connectivity.npz'.format
...
n_chips_required = naive_n_chips_calc(n_fibres / 10, n_ears,
                                      [(n_t, 255), (n_d, 255), (n_b, 255), (n_o, 255), (n_moc, 255)])
...
for ear_index in range(n_ears):
    ...
    input_pops[ear_index] = sim.Population(number_of_inputs, SpiNNakEar(audio_input=audio_data[ear_index], fs=Fs,
                                                                        ...
                                                                        param_file=spinnakear_param_file,
                                                                        ear_index=ear_index),
                                           label="AN_Pop_ear{}".format(ear_index))
    input_pops[ear_index].record(['spikes', 'moc'])

    d_pops[ear_index] = sim.Population(pop_size, sim.extra_models.Izhikevich_cond, d_stellate_izk_class_1_params,
    ...
    t_pops[ear_index] = sim.Population(pop_size, sim.extra_models.Izhikevich_cond, t_stellate_izk_class_2_params,
    ...
    b_pops[ear_index] = sim.Population(pop_size, sim.IF_cond_exp, bushy_params_cond,
                                       label="bushy_fixed_weight_scale_cond{}".format(ear_index))
    ...
    o_pops[ear_index] = sim.Population(pop_size, sim.extra_models.Izhikevich_cond, octopus_params_cond_izh,
                                       label="octopus_fixed_weight_scale_cond{}".format(ear_index))
    ...
    moc_pops[ear_index] = sim.Population(n_moc, sim.extra_models.Izhikevich_cond, d_stellate_izk_class_1_params,
                                         label="moc_fixed_weight_scale_cond{}".format(ear_index))
    ...
    if conn_pre_gen:
        ...
        if an_spatial:
            an_t_list = spatial_normal_dist_connection_builder(pop_size, number_of_inputs, n_t, RandomDistribution,
                                                               conn_num=n_an_t_connections, dist=1., sigma=1.
                                                               , conn_weight=an_t_weight)
        else:
            an_t_list = normal_dist_connection_builder(number_of_inputs, n_t, RandomDistribution,
                                                       conn_num=n_an_t_connections, dist=1., sigma=1.
                                                       , conn_weight=an_t_weight)
            # scale up post ids
    ...
    an_t_projs[ear_index] = sim.Projection(input_pops[ear_index], t_pops[ear_index], sim.FromListConnector(an_t_list),
    ...
    if conn_pre_gen:
        ...
        if an_spatial:
            an_d_list = spatial_normal_dist_connection_builder(pop_size, number_of_inputs, n_d, RandomDistribution,
                                                               conn_num=n_an_d_connections, dist=1.,
                                                               sigma=number_of_inputs / 15., conn_weight=an_d_weight)
        else:
            an_d_list = normal_dist_connection_builder(number_of_inputs, n_d, RandomDistribution,
                                                       conn_num=n_an_d_connections, dist=1.,
                                                       sigma=number_of_inputs / 15., conn_weight=an_d_weight)
            # scale up post ids
    ...
    an_d_projs[ear_index] = sim.Projection(input_pops[ear_index], d_pops[ear_index], sim.FromListConnector(an_d_list),
                                           synapse_type=sim.StaticSynapse())
    ...
    if conn_pre_gen:
        ...
        if an_spatial:
            an_b_list = spatial_normal_dist_connection_builder(pop_size, number_of_inputs, n_b, RandomDistribution,
                                                               conn_num=n_an_b_connections, dist=1.,
                                                               sigma=1., conn_weight=an_b_weight)
        else:
            an_b_list = normal_dist_connection_builder(number_of_inputs, n_b, RandomDistribution,
                                                       conn_num=n_an_b_connections, dist=1.,
                                                       sigma=1., conn_weight=an_b_weight)
            # scale up post ids
...
if lateral is True:
    for ear_index in range(n_ears):
        ...
        if conn_pre_gen:
            ...
            t_t_list = spatial_normal_dist_connection_builder(pop_size, n_t, n_t, RandomDistribution,
                                                              conn_num=n_lateral_connections, dist=1.,
                                                              sigma=t_lat_sigma, conn_weight=lat_t_weight)
        t_t_projs[ear_index] = sim.Projection(t_pops[ear_index], t_pops[ear_index], sim.FromListConnector(t_t_list),
                                              synapse_type=sim.StaticSynapse())
        if conn_pre_gen:
            t_d_list = connection_dicts[ear_index]['t_d_list']
        else:
            t_d_list = spatial_normal_dist_connection_builder(pop_size, n_t, n_d, RandomDistribution,
                                                              conn_num=d_t_ratio * n_lateral_connections, dist=1.,
                                                              sigma=d_lat_sigma, conn_weight=lat_d_weight)
        t_d_projs[ear_index] = sim.Projection(t_pops[ear_index], d_pops[ear_index], sim.FromListConnector(t_d_list),
                                              synapse_type=sim.StaticSynapse())
        if conn_pre_gen:
            t_b_list = connection_dicts[ear_index]['t_b_list']
        else:
            t_b_list = spatial_normal_dist_connection_builder(pop_size, n_t, n_b, RandomDistribution,
                                                              conn_num=t_b_ratio * n_lateral_connections, dist=1.,
                                                              sigma=b_lat_sigma, conn_weight=lat_b_weight)
        t_b_projs[ear_index] = sim.Projection(t_pops[ear_index], b_pops[ear_index], sim.FromListConnector(t_b_list),
                                              synapse_type=sim.StaticSynapse())
        ...
        if conn_pre_gen:
            ...
            d_d_list = spatial_normal_dist_connection_builder(pop_size, n_d, n_d, RandomDistribution,
                                                              conn_num=d_t_ratio * n_lateral_connections, dist=1.,
                                                              sigma=d_lat_sigma, conn_weight=lat_d_weight)
        ...
        if conn_pre_gen:
            ...
            d_t_list = spatial_normal_dist_connection_builder(pop_size, n_d, n_t, RandomDistribution,
                                                              conn_num=d_t_ratio * n_lateral_connections, dist=1.,
                                                              sigma=t_lat_sigma, conn_weight=lat_t_weight)
        d_t_projs[ear_index] = sim.Projection(d_pops[ear_index], t_pops[ear_index], sim.FromListConnector(d_t_list),
                                              synapse_type=sim.StaticSynapse(), receptor_type='inhibitory')
        if conn_pre_gen:
            d_b_list = connection_dicts[ear_index]['d_b_list']
        else:
            d_b_list = spatial_normal_dist_connection_builder(pop_size, n_d, n_b, RandomDistribution,
                                                              conn_num=d_b_ratio * n_lateral_connections, dist=1.,
                                                              sigma=b_lat_sigma, conn_weight=lat_b_weight)
        d_b_projs[ear_index] = sim.Projection(d_pops[ear_index], b_pops[ear_index], sim.FromListConnector(d_b_list),
                                              synapse_type=sim.StaticSynapse(), receptor_type='inhibitory')
        ...
        if n_ears > 1:
            ...
            if conn_pre_gen:
                ...
                d_tc_list = spatial_normal_dist_connection_builder(pop_size, n_d, n_t, RandomDistribution,
                                                                   conn_num=d_t_ratio * n_lateral_connections, dist=1.,
                                                                   sigma=t_lat_sigma, conn_weight=lat_t_weight)
                connection_dicts[ear_index]['d_tc_list'] = d_tc_list

            d_tc_projs[ear_index] = sim.Projection(d_pops[ear_index], t_pops[contra_ear_index],
                                                   sim.FromListConnector(d_tc_list), synapse_type=sim.StaticSynapse(),
            ...
            n_moc_an_connections = RandomDistribution('normal_clipped',
                                                      [av_n_moc_connections, 0.1 * av_n_moc_connections, 0,
                                                       av_n_moc_connections * 2.])
...
if conn_pre_gen is False:
    np.savez_compressed(input_directory + '/cn_{}x{}an_fibres_connectivity.npz'.format
...
for ear_index in range(n_ears):
    ...
    for i, neuron_times in enumerate(neuron_list):
        non_zero_neuron_times = neuron_times[
            ear_index]  # [spikes for spikes in neuron_times[ear_index] if len(spikes)>0]#
...
np.savez_compressed(input_directory + '/cn_' + test_file + '_{}an_fibres_{}ms_timestep_{}dB_lateral{}_moc_{}'.format
