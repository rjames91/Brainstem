...
#numpy.save("./spike_times_spinn_no5",spike_times_spinn)
test_spikes=[]
...
if num_repeats>1:
    ...
    for _ in range(num_repeats):
        for i in range(num_samples):
            ...
            spike_times_spinn=numpy.load("./spike_times_spinn_yes{}.npy".format(i+1))
            test_spikes.append(spike_times_spinn)
            spike_times_spinn=numpy.load("./spike_times_spinn_no{}.npy".format(i+1))
            test_spikes.append(spike_times_spinn)

    [connected_spike_trains,max_time] = spike_train_join(test_spikes,AN_pop_size)
    neuron_id=0
    spikes_train_an_ms=[]
    for neuron in connected_spike_trains:
        for time in neuron:
            spikes_train_an_ms.append((neuron_id,int(time)))
        neuron_id+=1

    sim_duration = max_time + 10.
    AN_pop = sim.Population(AN_pop_size, sim.SpikeSourceArray, {'spike_times': connected_spike_trains}, label='AN pop')
...
#AN --> CH connectivity==================================================================
an2ch_weight = RandomDistribution('uniform',parameters=[0.,2.])#10.#25.
#an2ch_fan = 5
diagonal_width = 26.
...
#AN --> ON connectivity==================================================================
an2on_prob=0.1#0.15#0.6#0.54#
an2on_weight =0.5#0.8#0.3#0.26#
...
#PL --> IC connectivity==================================================================
pl2ic_weight = RandomDistribution('uniform',parameters=[10,20])#5.#2.#10.#25.
...
if psth:
    ...
    #psth_plot(plt,numpy.arange(200,400),CH_spikes,bin_width=0.001,duration=duration,scale_factor=0.001,Fs=Fs,title="PSTH_CH")
