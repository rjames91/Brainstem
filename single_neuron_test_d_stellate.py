...
# d_ds = np.logspace(np.log10(20),np.log10(200),n_dds)
...
# results_data = np.load('./cn_tone_{}Hz_stereo_15s_30000an_fibres_0.1ms_timestep_50dB_16s_moc_True_lat_True.npz'.format(freq))
...
for dd in range(n_dds):
    ...
    d_stellate_izk_class_2_params['d'] = d_ds[dd]
    d_pops[dd] = sim.Population(n_sub_d, sim.Izhikevich, d_stellate_izk_class_2_params,
                                           label="d_stellate")
...
w2s_d = 15.#1.#0.5#0.1#0.75 # 0.3#0.5#
...
duration = 300.#max(input_spikes[0])
