...
tone_1_r = generate_signal(freq=1000,dBSPL=dBSPL/2.,duration=0.05,
                       modulation_freq=0.,fs=Fs,ramp_duration=0.005,plt=None,silence=True,silence_duration=0.075)
...
for ear_index in range(n_ears):
    ...
    if pop_size > number_of_inputs: # need to scale
        ...
        an_spatial = True
    input_pops[ear_index]=sim.Population(pop_size,SpiNNakEar(audio_input=audio_data[ear_index],fs=Fs,n_channels=pop_size/10),label="AN_Pop_ear{}".format(ear_index))
    ...
    if conn_pre_gen:
        ...
        if an_spatial:
            ...
            an_t_list = [(an,int(t*scale_factor),w,d) for (an,t,w,d) in an_t_list]
    ...
    if conn_pre_gen:
        ...
        if an_spatial:
            ...
            an_d_list = [(an,int(d*scale_factor),w,d) for (an,d,w,d) in an_d_list]
    ...
    if conn_pre_gen:
        ...
        if an_spatial:
            ...
            an_b_list = [(an,int(b*scale_factor),w,d) for (an,b,w,d) in an_b_list]
    ...
    if conn_pre_gen:
        ...
        if an_spatial:
            ...
            an_o_list = [(an,int(o*scale_factor),w,d) for (an,o,w,d) in an_o_list]
    ...
    for ear_index in range(n_ears):
        ...
        if n_ears>1:
            ...
            if conn_pre_gen:
                ...
                t_mocc_list = spatial_normal_dist_connection_builder(pop_size, n_t, n_moc, RandomDistribution,
                                                                     conn_num=n_t_moc_connections, dist=1.,
                                                                     sigma=pop_size / 100., conn_weight=t_moc_weight)
            ...
            if conn_pre_gen:
                ...
                #now scale-up the moc neuron indices but keep ohc indices the same
                scale_factor = float(pop_size-1)/(n_moc-1)
                moc_an_list = [(int(moc * scale_factor),ohc,w,d) for (moc,ohc,w,d) in moc_an_list]
            ...
            if conn_pre_gen:
                ...
                #now scale-up the moc neuron indices but keep ohc indices the same
                moc_anc_list = [(int(moc * scale_factor),ohc,w,d) for (moc,ohc,w,d) in moc_anc_list]
